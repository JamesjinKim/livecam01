...
async function toggleRecording(cameraId) {
    if (isApiCallInProgress) {
        console.log('[WARN] API 호출이 이미 진행 중입니다.');
        return;
    }

    isApiCallInProgress = true;
    const recordBtn = document.getElementById(`record-cam${cameraId}-btn`);

    try {
        if (recordingStates[cameraId]) {
            // 녹화 중지
            console.log(`[STOP] 카메라 ${cameraId} 녹화 중지 요청`);
            ...
            if (response.ok) {
                ...
                recordBtn.textContent = `카메라${cameraId} 녹화(30초)`;
                recordBtn.classList.remove('recording');
                console.log(`[OK] 카메라 ${cameraId} 녹화 중지됨`);
            } else {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
        } else {
            // 녹화 시작
            console.log(`[START] 카메라 ${cameraId} 녹화 시작 요청`);
            ...
            if (response.ok) {
                ...
                recordBtn.textContent = `카메라${cameraId} 녹화중...`;
                recordBtn.classList.add('recording');
                console.log(`[OK] 카메라 ${cameraId} 녹화 시작됨: ${result.message}`);
                ...
                setTimeout(() => {
                    if (recordingStates[cameraId]) {
                        ...
                        recordBtn.textContent = `카메라${cameraId} 녹화(30초)`;
                        recordBtn.classList.remove('recording');
                        console.log(`[AUTO] 카메라 ${cameraId} 녹화 자동 종료`);
                    }
                }, 30000);
            } else {
                throw new Error(`HTTP error! status: ${response.status}`);
        ...
        console.error(`[ERROR] 카메라 ${cameraId} 녹화 ${recordingStates[cameraId] ? '중지' : '시작'}에 실패했습니다:`, error);
        alert(`카메라 ${cameraId} 녹화 ${recordingStates[cameraId] ? '중지' : '시작'}에 실패했습니다: ${error.message}`);
    } finally {
