#!/usr/bin/env python3
"""
Picamera2 GPU 가속 카메라 1번 연속 녹화 시스템
30초씩 MP4 파일을 연속으로 저장하는 GPU 가속 레코더
...
class Picamera2Recorder:
    def __init__(self, camera_id=1, duration=31, resolution="640x480"):
        """
        ...
        # 저장 디렉토리 설정 (cam1용)
        self.base_dir = Path("videos/cam1")
        ...
        print("=" * 60)
        print("Picamera2 GPU 가속 카메라 1번 연속 녹화 시스템")
        print("=" * 60)
        ...
        try:
            ...
            video_config = self.picam2.create_video_configuration(
                main={
                    ...
                    "format": "YUV420"  # 카메라1은 YUV420 유지 (카메라0과 다른 포맷)
                },
    ...
    def _record_single_video(self, output_path):
        ...
        try:
            ...
            self.encoder = H264Encoder(
                bitrate=5000000,  # 5Mbps (480p에 적합)
...
def main():
    ...
    # 카메라 확인
    print("[TEST] 카메라 1번 확인 중...")
    
    try:
        # Picamera2로 카메라 테스트
        test_cam = Picamera2(camera_num=1)
        test_cam.close()
        print("[OK] 카메라 1번 정상 (Picamera2)")
